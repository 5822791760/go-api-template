...
import (
	...
	"github.com/5822791760/go-api-template/internal/db/postgres/public/model"
...
func (s *BookService) BuyBook(bookID int32, userID int32) (BuyBook, errs.ErrRenderer) {
	...
	if err := stmt.Query(tx, &returningBook); err != nil {
		return BuyBook{}, errs.NewErr(err, http.StatusInternalServerError)
	}

	if returningBook.Amount < 0 {
		return BuyBook{}, errs.NewErrByString("Not enough book in inventory", http.StatusBadRequest)
	}
	...
	if cash < 0 {
		return BuyBook{}, errs.NewErrByString("User don't have enough cash", http.StatusBadRequest)
	}
