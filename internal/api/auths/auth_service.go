...
func (s *AuthService) SignUp(body SignUpBody) (SignInToken, errs.ErrRenderer) {
	tx, _ := s.db.Begin()
	defer tx.Rollback()
	...
	if err := stmt.Query(tx, &returningA); err != nil {
		return SignInToken{}, errs.NewErr(err, http.StatusInternalServerError)
	}
...
func (s *AuthService) SignInByUserID(userID int32, password string) (SignInToken, errs.ErrRenderer) {
	tx, errA := s.db.Begin()
	if errA != nil {
		return SignInToken{}, errs.NewErr(errA, http.StatusInternalServerError)
	}
	defer tx.Rollback()
...
func (s *AuthService) SignInByUserEmail(email string, password string) (SignInToken, errs.ErrRenderer) {
	tx, errA := s.db.Begin()
	if errA != nil {
		return SignInToken{}, errs.NewErr(errA, http.StatusInternalServerError)
	}
